...
export function setupStencilSystem({ view, viewerDiv, contextRoot, originObject3D, destinationObject3D }) {
    ...
    function centerCylinderAtScreenCenter(stencil) {
        ...
        if (stencil === stencil2) {
            const p2 = mapFromContext(targetContext, destinationObject3D);
            stencil2.cylinder.setCenterECEF(p2);
        } else {
            const p1 = mapFromContext(targetContext, originObject3D);
            stencil1.cylinder.setCenterECEF(p1);
        }
    ...
    viewerDiv.addEventListener('click', (event) => {
        ...
        if (picked) {
            if (stencil1.picking) {
                ...
                repositionMaskWithRotation({ stencil: stencil1, targetWorld: target, globeRoot: originObject3D });
            }
            if (stencil2.picking) {
                const target = mapFromContext(picked, destinationObject3D);
                repositionMaskWithRotation({ stencil: stencil2, targetWorld: target, globeRoot: destinationObject3D });
            }
