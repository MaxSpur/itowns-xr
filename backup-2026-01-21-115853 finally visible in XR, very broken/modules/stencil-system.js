...
export function setupStencilSystem({ view, viewerDiv, contextRoot, originObject3D, destinationObject3D }) {
    ...
    const DEFAULT_MAP_RADIUS_METERS = 1500;
    const DEFAULT_CYLINDER_RADIUS = 0.125;
    const DEFAULT_GLOBE_SCALE = DEFAULT_CYLINDER_RADIUS / DEFAULT_MAP_RADIUS_METERS;
    const SCALE_MIN = 1 / 200000;
    const SCALE_MAX = 1 / 500;
    ...
    const stencil1 = {
        ...
        color: 0x2f8bff,
        uniforms: createStencilUniforms(DEFAULT_CYLINDER_RADIUS * originScale),
    ...
    const stencil2 = {
        ...
        color: 0xff3344,
        uniforms: createStencilUniforms(DEFAULT_CYLINDER_RADIUS * destinationScale),
    ...
    const stencil3 = {
        ...
        color: 0x2ecc71,
        uniforms: createStencilUniforms(DEFAULT_CYLINDER_RADIUS * contextScale),
    ...
    const pendingScale = { value: DEFAULT_GLOBE_SCALE };
    ...
    function updateRadiusLabelForScale(panel, idPrefix, baseScale, currentScale) {
        ...
        const radius = radiusFromSlider01(u) * baseScale / scale;
    ...
    function applyGlobeScale(nextScale) {
        const clamped = THREE.MathUtils.clamp(+nextScale || SCALE_MIN, SCALE_MIN, SCALE_MAX);
    ...
    scaleValue.textContent = formatScaleRatio(DEFAULT_GLOBE_SCALE);
    ...
    const setScaleUI = (scale) => {
        ...
        scaleInput.value = `${slider01FromScale(scale)}`;
    };
    setScaleUI(DEFAULT_GLOBE_SCALE);
    scaleInput.addEventListener('input', (e) => {
        const u = parseFloat(e.target.value);
        const scale = scaleFromSlider01(u);
        pendingScale.value = scale;
        scaleValue.textContent = formatScaleRatio(scale);
        if (centersReady) applyGlobeScale(scale);
    });
    ...
    view.addEventListener(itowns.GLOBE_VIEW_EVENTS.GLOBE_INITIALIZED, () => {
        initStencilCenters();
        centersReady = true;
        applyGlobeScale(pendingScale.value);
        setScaleUI(pendingScale.value);
