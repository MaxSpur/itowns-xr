...
export function setupStencilSystem({ view, viewerDiv, globe2Object3D, globe3Object3D }) {
    ...
    const alignmentState = { enabled: true, angleRad: 0 };
    const alignmentUI = { labels: [], buttons: [] };
...
>>>>>>> theirs
    ...
    function mapToContext(position, sourceRoot) {
        return mapCenterToGlobe(sourceRoot, globe3Object3D, position);
    }

    function getPickedContext(event) {
        const picked = view.getPickingPositionFromDepth(view.eventToViewCoords(event));
        if (!picked) return null;
        if (contextModeState.enabled) return picked;
        const globe1Root = getGlobe1Root();
        return globe1Root ? mapToContext(picked, globe1Root) : picked;
<<<<<<< ours
>>>>>>> theirs
=======
>>>>>>> theirs
    ...
    function computeAlignmentAngle() {
        const p1 = stencil1.uniforms.uStencilCenter.value;
        const p2 = stencil2.uniforms.uStencilCenter.value;
        if (!p1 || !p2) return 0;
        const up = p1.clone().normalize();
    ...
    function updateAlignmentUI() {
        const deg = THREE.MathUtils.radToDeg(alignmentState.angleRad);
        const text = `Alignment rotation: ${deg.toFixed(1)}°`;
        for (const label of alignmentUI.labels) label.textContent = text;
        for (const button of alignmentUI.buttons) {
            button.classList.toggle('is-active', alignmentState.enabled);
            button.textContent = alignmentState.enabled ? 'Auto align on' : 'Auto align off';
        }
    }

    function applyAlignment() {
        if (!alignmentState.enabled) return;
        const desired = computeAlignmentAngle();
        const delta = desired - alignmentState.angleRad;
        ...
        const globe1Root = getGlobe1Root();
        if (globe1Root) {
            const axis1 = p1.clone().normalize();
            rotateAroundPoint(globe1Root, p1.clone(), axis1, delta);
        }
        const axis2 = p2.clone().sub(globe2Object3D.position).normalize();
        rotateAroundPoint(globe2Object3D, p2.clone(), axis2, delta);
        alignmentState.angleRad = desired;
        updateAlignmentUI();
    }

    function setAlignmentEnabled(enabled) {
        alignmentState.enabled = !!enabled;
        if (!alignmentState.enabled && Math.abs(alignmentState.angleRad) > 1e-6) {
            const p1 = stencil1.uniforms.uStencilCenter.value;
            const p2 = stencil2.uniforms.uStencilCenter.value;
            if (p1 && p2) {
                const globe1Root = getGlobe1Root();
                if (globe1Root) {
                    const axis1 = p1.clone().normalize();
                    rotateAroundPoint(globe1Root, p1.clone(), axis1, -alignmentState.angleRad);
                }
                const axis2 = p2.clone().sub(globe2Object3D.position).normalize();
                rotateAroundPoint(globe2Object3D, p2.clone(), axis2, -alignmentState.angleRad);
            }
            alignmentState.angleRad = 0;
        }
        updateAlignmentUI();
...
<<<<<<< ours
        const globe1Root = getGlobe1Root();
        const q1 = globe1Root ? globe1Root.quaternion : identityQuat;
        tempQuat.copy(q1).slerp(globe2Object3D.quaternion, 0.5);
...
>>>>>>> theirs
    ...
    function rotateGlobe1ToTarget(targetECEF) {
        ...
        const globe1Root = getGlobe1Root();
        if (!desired || !globe1Root) return;
        const tDir = targetECEF.clone().normalize();
        const dDir = desired.clone().normalize();
        if (tDir.lengthSq() < 1e-6 || dDir.lengthSq() < 1e-6) return;

        const q = new THREE.Quaternion().setFromUnitVectors(tDir, dDir);
        globe1Root.quaternion.premultiply(q);
        globe1Root.updateMatrixWorld(true);
        view.notifyChange(true);
        updateGreenFromBlueRed();
    }
...
=======
        const globe1Root = getGlobe1Root();
        const targetContext = contextModeState.enabled
            ? target
            : (globe1Root ? mapToContext(target, globe1Root) : target);

        if (stencil === stencil2) {
            const p2 = mapFromContext(targetContext, globe2Object3D);
            stencil2.cylinder.setCenterECEF(p2);
        } else {
=======
        const globe1Root = getGlobe1Root();
...
>>>>>>> theirs
            const p1 = globe1Root ? mapFromContext(targetContext, globe1Root) : targetContext;
            stencil1.cylinder.setCenterECEF(p1);
>>>>>>> theirs
        }
    ...
    const alignLabel1 = document.createElement('div');
    alignLabel1.style.cssText = 'opacity:0.85;';
    const alignToggle1 = document.createElement('button');
    alignToggle1.style.cssText = UI_BUTTON_STYLE;
    alignToggle1.addEventListener('click', () => setAlignmentEnabled(!alignmentState.enabled));
    alignmentUI.labels.push(alignLabel1);
    alignmentUI.buttons.push(alignToggle1);
    const alignAnchor1 = stencil1.ui.panel.querySelector('input[type="range"]')?.parentElement;
    if (alignAnchor1) {
        stencil1.ui.panel.insertBefore(alignLabel1, alignAnchor1);
        stencil1.ui.panel.insertBefore(alignToggle1, alignAnchor1);
    } else {
        stencil1.ui.panel.appendChild(alignLabel1);
        stencil1.ui.panel.appendChild(alignToggle1);
    }
    ...
    const alignLabel2 = document.createElement('div');
    alignLabel2.style.cssText = 'opacity:0.85;';
    const alignToggle2 = document.createElement('button');
    alignToggle2.style.cssText = UI_BUTTON_STYLE;
    alignToggle2.addEventListener('click', () => setAlignmentEnabled(!alignmentState.enabled));
    alignmentUI.labels.push(alignLabel2);
    alignmentUI.buttons.push(alignToggle2);
    const alignAnchor2 = stencil2.ui.panel.querySelector('input[type="range"]')?.parentElement;
    if (alignAnchor2) {
        stencil2.ui.panel.insertBefore(alignLabel2, alignAnchor2);
        stencil2.ui.panel.insertBefore(alignToggle2, alignAnchor2);
    } else {
        stencil2.ui.panel.appendChild(alignLabel2);
        stencil2.ui.panel.appendChild(alignToggle2);
    }
    ...
    resetBtn.addEventListener('click', () => {
        const globe1Root = getGlobe1Root();
        if (globe1Root) {
            globe1Root.quaternion.copy(globe3Object3D.quaternion);
            globe1Root.updateMatrixWorld(true);
        }
        globe2Object3D.quaternion.copy(globe3Object3D.quaternion);
        globe2Object3D.updateMatrixWorld(true);
        view.notifyChange(true);
        updateGreenFromBlueRed();
    });
...
<<<<<<< ours
        ...
        if (picked) {
            if (stencil1.picking) {
                ...
                repositionMaskWithRotation({ stencil: stencil1, targetWorld: target, globeRoot: getGlobe1Root() });
            }
            if (stencil2.picking) {
                const target = contextModeState.enabled ? mapFromContext(picked, globe2Object3D) : picked;
                repositionMaskWithRotation({ stencil: stencil2, targetWorld: target, globeRoot: globe2Object3D });
            }
            if (stencil2.rotating) {
                const target = contextModeState.enabled ? mapFromContext(picked, globe2Object3D) : picked;
=======
        const pickedContext = getPickedContext(event);
        if (pickedContext) {
            const globe1Root = getGlobe1Root();
            if (stencil1.picking) {
                const target = globe1Root ? mapFromContext(pickedContext, globe1Root) : pickedContext;
                repositionMaskWithRotation({ stencil: stencil1, targetWorld: target, globeRoot: globe1Root });
            }
            if (stencil2.picking) {
                const target = mapFromContext(pickedContext, globe2Object3D);
                repositionMaskWithRotation({ stencil: stencil2, targetWorld: target, globeRoot: globe2Object3D });
            }
            if (stencil2.rotating) {
                const target = mapFromContext(pickedContext, globe2Object3D);
